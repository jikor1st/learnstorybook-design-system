...
- Auto 설치
  ...
  Auto에서 깃허브(GitHub)와 npm 통신을 해야합니다. 이 과정에서 토큰이 필요합니다.
  이 토큰은 repo 스코프를 필요로 합니다.

  > github 토큰 발행 페이지 : https://github.com/settings/tokens

  npm 토큰은 "읽기와 배포"의 허가에 대한 토큰을 발행해야 합니다.

  > npm 토큰 발행 페이지 : https://www.npmjs.com/settings/&lt;your-username&gt;/tokens
  ...
  Auto로 깃허브에 라벨 종류 생성
  -> 라벨은 패키지를 수정할 때 사용하는데, auto가 패키지 버전을 합리적이고 업데이트하고 changelog와 릴리즈 노트를 생성할 수 있습니다.
